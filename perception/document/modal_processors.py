...
class ImageModalProcessor(BaseModalProcessor):
    ...
    async def generate_description_only(
        ...
        try:
            ...
            # Build data URI for the image to comply with BLTCY interface
            import mimetypes
    ...
    def _parse_response(
        ...
        try:
            ...
            # Accept multiple possible fields for description
            description = ""
            if isinstance(response_data, dict):
                ...
                entity_data = response_data.get("entity_info", {}) or {}
            else:
                entity_data = {}

            # If description still missing, fall back to raw text
            if not description:
                description = (
                    response
            ...
            # If entity_info missing required fields, synthesize a fallback
            if not isinstance(entity_data, dict) or not all(
                key in entity_data for key in ["entity_name", "entity_type", "summary"]
            ):
                fallback_entity = {
                    "entity_name": entity_name
                    if entity_name
                    else f"image_{compute_mdhash_id(description)}",
                    "entity_type": "image",
                    "summary": description[:100] + "..." if len(description) > 100 else description,
                }
                return description, fallback_entity
            ...
            fallback_entity = {
                "entity_name": entity_name
                if entity_name
                else f"image_{compute_mdhash_id(str(response))}",
                "entity_type": "image",
                "summary": str(response)[:100] + "..." if len(str(response)) > 100 else str(response),
            }
            return str(response), fallback_entity
