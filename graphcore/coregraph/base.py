...
class OllamaServerInfos:
    def __init__(self, name=None, tag=None):
        self._graphcore_name = name or os.getenv(
            "OLLAMA_EMULATING_MODEL_NAME", DEFAULT_OLLAMA_MODEL_NAME
        )
        self._graphcore_tag = tag or os.getenv(
    ...
    def LIGHTRAG_NAME(self):
        return self._graphcore_name

    @LIGHTRAG_NAME.setter
    def LIGHTRAG_NAME(self, value):
        self._graphcore_name = value

    @property
    def LIGHTRAG_TAG(self):
        return self._graphcore_tag

    @LIGHTRAG_TAG.setter
    def LIGHTRAG_TAG(self, value):
        self._graphcore_tag = value

    @property
    def LIGHTRAG_MODEL(self):
        return f"{self._graphcore_name}:{self._graphcore_tag}"
...
@dataclass
class QueryParam:
    """Configuration parameters for query execution in GraphCore."""
    ...
    entity_retrieval: Literal["embedding", "bm25", "hybrid"] = os.getenv(
        "ENTITY_RETRIEVAL", "hybrid"
    )
    """Retrieval policy for entities. When set to "bm25" the system prefers lexical search,
    "embedding" keeps the original behaviour, and "hybrid" tries BM25 first before
    falling back to embeddings."""

    relation_retrieval: Literal["embedding", "bm25", "hybrid"] = os.getenv(
        "RELATION_RETRIEVAL", "hybrid"
    )
    """Retrieval policy for relationships/hyperedges."""

    chunk_retrieval: Literal["embedding", "bm25", "hybrid"] = os.getenv(
        "CHUNK_RETRIEVAL", "hybrid"
    )
    """Retrieval policy for free-form chunks."""
