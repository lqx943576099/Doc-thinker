...
logger = logging.getLogger("graphcore.coregraph")
...
if TYPE_CHECKING:
    from graphcore.coregraph.base import BaseKVStorage, BaseVectorStorage, QueryParam
...
class LightragPathFilter(logging.Filter):
    """Filter for graphcore.coregraph logger to filter out frequent path access logs"""
...
def setup_logger(
    ...
    Args:
        ...
        log_file_path: Path to the log file. If None and file logging is enabled, defaults to graphcore.coregraph.log in LOG_DIR or cwd
...
class EmbeddingFunc:
    ...
    async def __call__(self, *args, **kwargs) -> np.ndarray:
        ...
        if args and isinstance(args[0], (list, tuple)):
            ...
            if actual_vectors != expected_vectors and expected_vectors > 0:
                logger.warning(
                    "Embedding vector count mismatch: expected %s vectors, got %s. "
                    "Adjusting embeddings to match expected count.",
                    expected_vectors,
                    actual_vectors,
                )

                # Reshape to (actual_vectors, dim)
                result = result.reshape(actual_vectors, expected_dim)

                if actual_vectors == 1:
                    # Broadcast single vector to all inputs
                    result = np.repeat(result, expected_vectors, axis=0)
                elif actual_vectors > expected_vectors:
                    # Truncate extra vectors
                    result = result[:expected_vectors]
                else:  # actual_vectors < expected_vectors
                    # Cycle through existing vectors to pad up to expected count
                    reps = (expected_vectors + actual_vectors - 1) // actual_vectors
                    result = np.vstack([result] * reps)[:expected_vectors]
...
async def aexport_data(
    ...
    if file_format == "csv":
        ...
        with open(output_path, "w", encoding="utf-8") as mdfile:
            mdfile.write("# GraphCore Data Export\n\n")
...
def check_storage_env_vars(storage_name: str) -> None:
    ...
    """
    from graphcore.coregraph.kg import STORAGE_ENV_REQUIREMENTS
