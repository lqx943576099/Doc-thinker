...
- Do NOT include any explanation, comments, or additional text outside the JSON.
- Do NOT wrap the JSON with extra Markdown text, except a single ```json code block if needed.
- Use only double quotes (") for all keys and string values.
- Do NOT use trailing commas.
- Do NOT use NaN, Infinity, or undefined; use null instead if needed.
- If you are unsure, still return the best JSON you can; never return plain text only.
...
# Custom VLM vision prompt for image description
PROMPTS["vision_prompt_v2"] = """ 
 Please generate an enhanced visual description based on my query,and provide a JSON response with the following structure: 
 
 { 
   "detailed_description": "A comprehensive and detailed visual description based on my query of the image following these guidelines: 
   - Identify all objects, people, text, and visual elements 
   - Explain relationships between elements 
   - Note colors, lighting, and visual style 
   - Describe any actions or activities shown 
   ...
   "image_description": { 
     "type": "image", 
     "description": "Concise summary of the image content and its significance" 
   } 
 } 
