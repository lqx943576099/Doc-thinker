#封装了各类LLM，embedding的api调用，以及各参数。
import base64
import copy
import json
import os
...
async def openai_complete_if_cache(
    ...
    **kwargs,
    ...
    resolved_api_key = api_key or os.getenv("LLM_BINDING_API_KEY") or os.getenv("OPENAI_API_KEY")
    if resolved_api_key:
        os.environ["OPENAI_API_KEY"] = resolved_api_key
    resolved_base_url = base_url or os.getenv("LLM_BINDING_HOST") or os.getenv("OPENAI_BASE_URL")

    timeout_cfg = Timeout(total=180)  # 或者 Timeout(connect=30, read=150)
    openai_async_client = (
        AsyncOpenAI(timeout=timeout_cfg)
        if resolved_base_url is None
        else AsyncOpenAI(base_url=resolved_base_url, timeout=timeout_cfg)
    )
    ...
    if "response_format" in kwargs and "qwen" in str(model).lower():
...
async def openai_complete(
    ...
    if keyword_extraction:
        if "qwen" in model_name.lower():
            pass  # do not set response_format for qwen models
        else:
...
async def openai_embedding(
    ...
    if resolved_api_key:
        os.environ["OPENAI_API_KEY"] = resolved_api_key
    resolved_base_url = base_url or os.getenv("EMBEDDING_BINDING_HOST") or os.getenv("LLM_BINDING_HOST") or os.getenv("OPENAI_BASE_URL")

    openai_async_client = (
        AsyncOpenAI() if resolved_base_url is None else AsyncOpenAI(base_url=resolved_base_url)
    )
...
async def qwen3_embedding(
    ...
    *,
    ...
    async with aiohttp.ClientSession(timeout=client_timeout) as session:
        async with session.post(resolved_base, headers=headers, json=payload) as resp:
    ...
    if not isinstance(data_items, list):
        raise ValueError(f"Unexpected embedding payload: {response_json}")

    embeddings: list[list[float]] = []
    for idx, item in enumerate(data_items):
        embedding = item.get("embedding")
        if embedding is None:
            raise ValueError(f"Missing embedding at index {idx}: {item}")
    ...
    if num_embeddings != num_texts and num_texts > 0:
        ...
        # Heuristic 1: API returned a single embedding for multiple inputs.
        # Replicate the single vector for each input text so downstream code
        #仍然能够获得“每段文本一个向量”的形状。
        if num_embeddings == 1:
            embeddings = [embeddings[0] for _ in range(num_texts)]
            num_embeddings = num_texts

        # Heuristic 2: API returned multiple chunks per input text (e.g. late chunking).
        # When the total embeddings is an exact multiple of num_texts, average each group.
        elif num_embeddings > num_texts and num_embeddings % num_texts == 0:
            arr = np.array(embeddings, dtype=np.float32)
            group = num_embeddings // num_texts
            arr = arr.reshape(num_texts, group, -1).mean(axis=1)
            return arr.astype(np.float32)

        # Fallback: make sure we always return exactly num_texts vectors by
        # truncating or cycling the embeddings list as needed.
        if num_embeddings != num_texts and num_embeddings > 0:
            if num_embeddings > num_texts:
                embeddings = embeddings[:num_texts]
            else:  # num_embeddings < num_texts
                embeddings = [embeddings[i % num_embeddings] for i in range(num_texts)]
