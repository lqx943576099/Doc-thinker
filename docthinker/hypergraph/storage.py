#建立KV，NanoVDB，NetworkXStorage等向量库，图等信息
import asyncio
import html
import os
...
class NanoVectorDBStorage(BaseVectorStorage):
    ...
    async def upsert(self, data: dict[str, dict]):
        ...
        async def wrapped_task(batch):
            timeout_sec = self.global_config.get("embedding_timeout", 360)
            try:
                result = await asyncio.wait_for(
                    self.embedding_func(batch), timeout=timeout_sec
                )
            except asyncio.TimeoutError as exc:
                logger.error(
                    "Embedding batch timed out after %ss (batch size=%s)",
                    timeout_sec,
                    len(batch),
                )
                raise exc
