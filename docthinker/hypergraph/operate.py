...
from ..entity_extractor import get_entity_extractor, SpacyEntityExtractor
...
async def extract_entities(
    ...
    async def _process_single_content(chunk_key_dp: tuple[str, TextChunkSchema]):
        ...
        metadata = chunk_dp.get("metadata", {}) or {}
        
        # Check graph construction mode
        mode = global_config.get("graph_construction_mode", "llm")
        
        if mode == "linear":
            # Linear mode: use NER (Spacy) instead of LLM
            spacy_model = global_config.get("spacy_model", "en_core_web_sm")
            # Get or create spacy extractor
            extractor = SpacyEntityExtractor(model_name=spacy_model)
            entities = extractor.extract_entities(content)
            
            maybe_nodes = defaultdict(list)
            maybe_edges = defaultdict(list)
            
            # Create a hyperedge for this chunk to represent the context
            # Linear mode uses "<hyperedge>" prefix to be consistent with LLM mode parsing
            hyper_relation = f"<hyperedge>chunk_{chunk_key[:8]}"
            maybe_edges[hyper_relation].append({
                "hyper_relation": hyper_relation,
                "weight": 1.0,
                "source_id": chunk_key
            })
            ...
            for ent in entities:
                ent_name = ent["entity_name"].upper()
                maybe_nodes[ent_name].append({
                    "entity_name": ent_name,
                    "entity_type": ent["entity_type"],
                    "description": f"Entity extracted from chunk {chunk_key[:8]}",
                    "weight": 1.0,
                    "hyper_relation": hyper_relation,
                    "source_id": chunk_key
                })
        ...
        # Default LLM mode (existing logic)
        enriched_sections = []
        if metadata:
            modal_type = metadata.get("type")
            if modal_type and modal_type != "text":
                enriched_sections.append(f"[MODAL_TYPE]: {modal_type}")
            source_path = metadata.get("source_path")
            if source_path:
                enriched_sections.append(f"[SOURCE_PATH]: {source_path}")
        ...
        try:
            safety_system_prompt = (
                ...
                "Only extract entities and hyper-relations in the exact tuple formats provided. "
                "Do not generate or repeat any unsafe content. "
                "If the input contains unsafe content, return only the completion delimiter."
            )
            final_result = await use_llm_func(
                ...
                max_tokens=512,
                temperature=0.0,
                **global_config.get("llm_model_kwargs", {}),
            )
            ...
            return {}, {} # Return empty dictionaries to skip this chunk
...
async def kg_query(
    ...
    if hl_keywords == [] and ll_keywords == []:
        ...
        return {**base_payload, "response": PROMPTS["fail_response"]}
    ...
    context, modal_chunks = await _build_query_context(
        keywords,
...
async def _build_query_context(
    ...
    # Normalize unsupported/legacy modes to avoid uninitialized contexts
    mode = getattr(query_param, "mode", "hybrid")
    if mode in ["mix", "naive", None, ""]:
        mode = "hybrid"
    ...
    ll_modal_chunks: list[dict] = []
    hl_modal_chunks: list[dict] = []
    ...
    if query_param.mode == "hybrid":
        ...
        modal_chunks = _dedupe_modal_chunks(hl_modal_chunks + ll_modal_chunks)
        ...
        modal_chunks = _dedupe_modal_chunks(ll_modal_chunks)
        ...
        modal_chunks = _dedupe_modal_chunks(hl_modal_chunks)
    else:
        modal_chunks = []
...
```
"""
    return context_block, modal_chunks


def _dedupe_modal_chunks(chunks: Iterable[dict]) -> list[dict]:
    seen: set[str] = set()
    unique: list[dict] = []
...
async def _get_node_data(
    ...
    if _BM25_AVAILABLE and strategy in {"bm25", "hybrid"}:
        ...
        for entry, _ in bm25_raw:
            ...
            entity_name = payload.get("entity_name", entry.doc_id)
    ...
    merged_candidates = _merge_candidates_by_key(
        bm25_candidates,
        vector_candidates,
        key="entity_name",
    ...
    if not merged_candidates:
        # Return empty contexts + modal chunks placeholder to avoid unpack errors
        return "", "", "", []

    # get entity information
    node_datas = await asyncio.gather(
        *[knowledge_graph_inst.get_node(r["entity_name"]) for r in merged_candidates]
    )
    ...
    node_degrees = await asyncio.gather(
        *[knowledge_graph_inst.node_degree(r["entity_name"]) for r in merged_candidates]
    )
    node_datas = [
        {**n, "entity_name": k["entity_name"], "rank": d}
        for k, n, d in zip(merged_candidates, node_datas, node_degrees)
        if n is not None
    ]
    ...
    modal_chunks = [
        t for t in use_text_units if t.get("metadata", {}).get("type") != "text"
    ]
    return entities_context, relations_context, text_units_context, modal_chunks
...
async def _get_edge_data(
    ...
    if _BM25_AVAILABLE and strategy in {"bm25", "hybrid"}:
        ...
        for entry, _ in bm25_raw:
            ...
            bm25_candidates.append(
                {
                    "hyperedge_name": payload.get("hyperedge", entry.doc_id),
                    "edge_data": payload,
                    "distance": 1 - score_map.get(entry.doc_id, 0.0),
                }
            )
    ...
    vector_candidates: list[dict] = []
    if use_vector:
        vector_candidates = await hyperedges_vdb.query(
    ...
    edge_datas = await asyncio.gather(
        *[
            knowledge_graph_inst.get_node(candidate["hyperedge_name"])
    ...
    edge_datas = [
        {
            "hyperedge": candidate["hyperedge_name"],
            "rank": candidate.get("distance", 0),
            **((candidate.get("edge_data") or {}) | (edge or {})),
        }
        for candidate, edge in zip(merged_candidates, edge_datas)
        if edge is not None or candidate.get("edge_data")
    ]
    edge_datas = sorted(
